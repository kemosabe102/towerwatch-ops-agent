...
def build_site_enum(names: list[str]) -> type[Enum]:
    ...
    rather than "the config failed to parse" (ADR-0007's observability requirement).
    """
    if not names:
        raise ValueError(
    ...
    return Enum("Site", {name: name for name in names}, type=str)  # type: ignore[return-value]
